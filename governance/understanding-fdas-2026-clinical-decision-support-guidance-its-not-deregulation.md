...
It does not acquire, process, or analyze medical images, signals from diagnostic devices, or patterns from signal acquisition systems. It does display, analyze, or print medical information about patients or other medical information. It does provide recommendations (not directives) to support physician decision-making. And it does enable the physician to independently review the basis for recommendations, not rely primarily on them.
...
Non-Device CDS examples that meet all four criteria include drug-drug interaction alerts based on patient records, treatment option lists derived from clinical guidelines, reminder systems for preventive care, and discharge planning assistance tools.

Regulated Device Software examples that fail one or more criteria include software analyzing CT images to detect abnormalities, algorithms processing ECG waveforms to diagnose arrhythmias, tools analyzing genetic sequences to identify mutations, and systems that provide specific diagnostic outputs in time-critical situations.
...
For software to qualify as non-device CDS, it must enable physicians to independently review and understand the purpose and intended use, the input data requirements and quality standards, a plain-language description of the underlying algorithm, the data used to develop and validate the algorithm, clinical study results demonstrating performance and limitations, and patient-specific information relevant to the recommendation.
...
Software that analyzes images, waveforms, or genomic patterns; provides diagnostic conclusions rather than options; operates in time-critical contexts; or cannot enable independent review of its reasoning will remain under full regulatory oversight. And rightfully so.
